...
TOP GRAPHIC: reproduce the reference graphic's STYLE and layout closely — same typography style (bold sans slogan / gothic / script / collage / character), same placement and scale — but make the WORDS/art ORIGINAL (not the exact brand wordmark). The text/art must be FORWARD and clearly READABLE — NEVER mirrored, reversed or backwards, even if the reference is a mirror selfie. It must be ORIGINAL/generic, never a recognizable THIRD-PARTY brand logo (no Stüssy, Supreme, Nike, BBC, Pleasures, Carhartt, Hellstar, etc.). Reference style DNA: ${refDna}`
  ...
  const heroNote = mode === 'carouselHero'
    ...
    : mode === 'video'
    ? `\nVIDEO-READY MIRROR-SELFIE (this still will be ANIMATED, and the PRODUCT must be the hero): frame it as a vertical phone MIRROR-SELFIE fit-check. VERY CLOSE CROP — the camera is CLOSE and the model's BODY FILLS the frame: top edge at about the CHIN/mouth (head cropped out, phone + lower face at the very top), bottom edge at the FEET. The model fills almost the full WIDTH too — only a sliver of room visible on the sides, NOT a wide full-room shot with empty space. The DENIM SHORTS and legs are LARGE and DOMINANT, filling the center and lower two-thirds, sharp and well lit. Just a hint of a clean minimal room behind for a little parallax. The model is calm and STILL (no walking), weight even, free hand relaxed. Think a real IG "outfit of the day" mirror clip shot up-close, product-forward.
SHORTS LENGTH IS CRITICAL: the denim shorts must end EXACTLY AT THE KNEE, identical to the product photo — do NOT render them longer / below the knee / as long shorts. Keep the exact wash, raw frayed hem, width and knee length of the product.`
    : '';
