...
TOP GRAPHIC: if the reference DNA describes a graphic/print on the top, design an ORIGINAL graphic in that SAME THEME and STYLE (e.g. ref says "money/$100-bill print" -> an original currency-themed art; "skateboard collage" -> your own photo-collage; "cartoon character" -> an original character) — a bold, cool, current streetwear graphic, NOT a plain blank. It must be ORIGINAL/generic, never a recognizable THIRD-PARTY brand logo, wordmark or copyrighted character (no Stüssy, Supreme, Nike, BBC, Icecream, Broken Planet, Pleasures, Carhartt, Hellstar, etc.). Keep brand attention on CAROTA. Reference style DNA: ${refDna}`
