...
TOP GRAPHIC: reproduce the reference graphic's STYLE and layout closely — same typography style (bold sans slogan / gothic / script / collage / character), same placement and scale — but make the WORDS/art ORIGINAL (not the exact brand wordmark). The text/art must be FORWARD and clearly READABLE — NEVER mirrored, reversed or backwards, even if the reference is a mirror selfie. It must be ORIGINAL/generic, never a recognizable THIRD-PARTY brand logo (no Stüssy, Supreme, Nike, BBC, Pleasures, Carhartt, Hellstar, etc.). Reference style DNA: ${refDna}`
  ...
  const heroNote = mode === 'carouselHero'
    ...
    : mode === 'video'
    ? `\nVIDEO-READY MIRROR-SELFIE (this still will be ANIMATED, and the PRODUCT must be the hero): frame it as a vertical phone MIRROR-SELFIE fit-check. CLOSE CROP — frame TIGHT, from about the CHIN/mouth DOWN to the FEET (the top of the head is cropped OUT of frame; the phone and the lower face sit at the very top edge). This makes the DENIM SHORTS and legs DOMINATE the center and lower two-thirds of the frame and command all the attention — fill the frame with the BODY, not the room. The shorts are centered, fully visible waist-to-hem with the legs and sneakers, well lit and sharp. Just enough of a clean minimal room (bedroom / apartment) with soft daylight behind for a little parallax. The model is calm and STILL (no walking), weight even, free hand relaxed. Exactly the close, product-forward energy of a real "outfit of the day" mirror clip — NOT a wide full-room shot.`
    : '';
