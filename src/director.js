...
TOP GRAPHIC: reproduce the reference graphic's STYLE and layout closely — same typography style (bold sans slogan / gothic / script / collage / character), same placement and scale — but make the WORDS/art ORIGINAL (not the exact brand wordmark). The text/art must be FORWARD and clearly READABLE — NEVER mirrored, reversed or backwards, even if the reference is a mirror selfie. It must be ORIGINAL/generic, never a recognizable THIRD-PARTY brand logo (no Stüssy, Supreme, Nike, BBC, Pleasures, Carhartt, Hellstar, etc.). Reference style DNA: ${refDna}`
  ...
  const heroNote = mode === 'carouselHero'
    ...
    : mode === 'video'
    ? `\nVIDEO-READY MIRROR-SELFIE (this still will be ANIMATED, and the PRODUCT must be the hero): frame it as a vertical phone MIRROR-SELFIE fit-check — the model stands facing a mirror holding the phone up near the chest/face (this safely hides part of the face and reads native to IG). Full body HEAD-TO-TOE in frame, fairly frontal and centered. The DENIM SHORTS are the clear FOCUS of the shot — centered, fully visible waist-to-hem with the legs and sneakers, well lit, sharp. A clean, minimal, well-lit room (bedroom / apartment) with soft natural daylight and a little depth behind for parallax. The model is calm and STILL (no mid-action, no walking), weight even, the free hand relaxed. No motion blur, no crop at the legs. Exactly the energy of a real "outfit of the day" mirror clip.`
    : '';
