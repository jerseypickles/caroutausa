...
const EXTRACT = {
  outfit: {
    sys: `You are a streetwear stylist for a youth/street brand. Look at this fitpic and extract its STYLE DNA so it can be REPRODUCED closely. Capture what makes the look — the lane/vibe, the top type, the FIT/SILHOUETTE, the GRAPHIC on the top, the SNEAKERS (exact model+colorway), the ACCESSORIES, the model's vibe/energy, and the palette. If the photo is a MIRROR SELFIE, read everything (especially text/graphics) the RIGHT way around. Ignore the bottoms (we use our own denim shorts) and the background.`,
    ask: 'Return ONLY JSON with short values: {"lane":"the vibe/caliber + the kinds of brands","top":"top/layer type","graphic":"the ARTWORK/PRINT on the top — describe its THEME, TYPOGRAPHY STYLE and layout so a similar ORIGINAL one can be designed (e.g. \\"bold black sans-serif slogan, centered\\", \\"money/$100-bill print\\", \\"skateboard photo-collage\\", \\"cartoon character\\") — IGNORE any brand name. If it is a MIRROR SELFIE and text reads backwards, describe it FORWARD (the right way around) — NEVER say upside-down/mirrored/reversed. Write \\"none\\" if blank","fit":"the SILHOUETTE/FIT — e.g. fitted/slim, regular, oversized/boxy, baggy/relaxed","sneakers":"the EXACT sneakers (model + colorway, e.g. \\"Balenciaga Triple S black/white\\")","accessories":"caps, chains, socks, watch, bag, etc.","vibe":"the model energy/casting vibe","palette":"main colors","family":"classify the lane into ONE word: edge, clean, or versatile"}',
